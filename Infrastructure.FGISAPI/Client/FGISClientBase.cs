...
internal abstract class FGISClientBase
{
    private readonly ILogger _logger;
    private readonly HTTPQueueManager _httpQueueManager;
    private readonly HttpClient _httpClient;

    public FGISClientBase(ILogger logger, HTTPQueueManager httpQueueManager, HttpClient httpClient)
    {
        _logger = logger;
        _httpQueueManager = httpQueueManager;
        _httpClient = httpClient;
    }
    protected async Task<T?> GetItemListAsync<T>(string endpoint, string query)
    {
        return await RequestBaseAsync<T>($"/{endpoint}", query);
    }

    protected async Task<T?> GetItemListAsync<T>(string endpoint, string search, uint? rows = null)
    {
        var query = BuildQuery(search, rows: rows);
        return await RequestBaseAsync<T>($"/{endpoint}", query);
    }

    protected async Task<T?> GetItemAsync<T>(string endpoint, string itemId)
    {
    ...
    private async Task<TResponse?> RequestBaseAsync<TResponse>(string uri, string query)
    {
        ...
        throw new Exception("We cant reach this state. To compiler shut up.");
    }
}